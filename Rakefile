...
lib_dir = File.join(base_dir, 'hroonga', 'lib')
$LOAD_PATH.unshift(lib_dir)

def guess_version
  require 'hroonga/version'
  Hroonga::VERSION
end
...
Jeweler::Tasks.new do |_spec|
  ...
  spec.email = emails
  spec.summary = "A HTTP interface for groonga."
  spec.description = <<-EOD.gsub(/\n/, ' ').strip
Hroonga is a HTTP interface for groonga implemented as Rack
middlewares. It's based on rroonga.
EOD
  spec.license = "LGPLv2"
